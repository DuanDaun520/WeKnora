...
注册接口可通过环境变量 `DISABLE_REGISTRATION=true` 关闭。密码策略仅要求**至少 6 位**，无字符类别等其他要求（复杂密码开关已随纯长度策略退役）。
...
修改当前用户的登录密码。新密码须满足**至少 6 位**（无字符类别要求）。不能与当前密码相同。成功后**所有会话被撤销**，需使用新密码重新登录。
...
| new_password  | string | 是   | 至少 6 位，且不同于旧密码 | 新密码    |
