...
// CreateTenant godoc
// @Summary      创建空间（仅系统管理员）
// @Description  企业化收权后仅系统管理员（或 platform API key）可创建空间；
// @Description  普通用户与空间的绑定由系统管理员通过 /system/admin 端点配置。
// @Description  创建者会被自动写入成员关系；tenantless 的创建者会把新空间
// @Description  设为默认登录空间。
...
func (h *TenantHandler) CreateTenant(c *gin.Context) {
	...
	// 企业化收权：只有系统管理员（或 platform API key）可以创建空间。
	// CanAccessAllTenants 保留为跨空间"读"通道，不再隐含建空间权；
	// 普通用户与空间的绑定统一由系统管理员通过 /system/admin 端点配置。
	catalogManager := caller.IsSystemAdmin || platformCaller
	if !catalogManager {
		logger.Warnf(ctx, "Tenant creation denied (system admin only) for user %s", caller.ID)
	...
	// Catalog-manager path (system admin / platform API key): accept the
	// full Tenant payload (status, storage_quota, retriever engines,
	// configs...) so existing tooling keeps working.
	...
	// caller didn't specify one. We resolve at create time on purpose —
	// the on-disk row should
	...
	// Bootstrap a membership so the creating system admin immediately has
	// access to the workspace they created. We MUST roll the tenant back
	// if this fails: without a membership row the new tenant is
	...
	// Idempotent: EnsureMember is a no-op when the row already exists.
	// (Platform API keys skip this — they have no user row to bind.)
	if h.memberService != nil && !platformCaller {
		if _, err := h.memberService.EnsureMember(ctx, caller.ID, createdTenant.ID); err != nil {
			logger.Errorf(ctx,
				"Failed to bootstrap membership for user %s tenant %d: %v — rolling back tenant",
			...
			c.Error(errors.NewInternalServerError("Failed to finalise workspace membership").WithDetails(err.Error()))
			return
		}
	}

	// When a tenantless creator (a freshly-bootstrapped system admin) makes
	// their first workspace, make it their default login tenant. Roll the
	// just-created resources back if this finalisation fails so the user
	// is not left with an unreachable tenant.
