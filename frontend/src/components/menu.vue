                ...
                <t-tooltip :content="item.title" placement="right" :disabled="!uiStore.sidebarCollapsed">
                    <div @click="handleMenuClick(item.path)" @mouseenter="mouseenteMenu(item.path)"
                        ...
                        <div class="menu_item-box">
                            <div class="menu_icon">
                                <img class="icon"
                                    :src="getImgSrc(item.icon == 'zhishiku' ? knowledgeIcon : item.icon == 'agent' ? agentIcon : item.icon == 'skillsMcp' ? skillsIcon : item.icon == 'organization' ? organizationIcon : item.icon == 'logout' ? logoutIcon : item.icon == 'setting' ? settingIcon : item.icon == 'user' ? consoleIcon : prefixIcon)"
                                    alt="">
                            </div>
...
const isMenuItemActive = (itemPath: string): boolean => {
    ...
    switch (itemPath) {
        ...
        case 'skills-mcp':
            return currentRoute === 'skillsMcpList';
        case 'organizations':
...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'skills-mcp' || item.path === 'organizations' || item.path === 'creatChat'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'skills-mcp' || item.path === 'organizations' || item.path === 'creatChat') {
            return false;
        }
        return true;
    });
...
const getIcon = (path: string) => {
    ...
    const skillsMcpActiveState = route.name === 'skillsMcpList';
    ...
    // Skills/MCP 图标：只在 Skills/MCP 页面显示绿色
    skillsIcon.value = skillsMcpActiveState ? 'skills-green.svg' : 'skills.svg';
...
const handleMenuClick = async (path: string) => {
    ...
    } else if (path === 'skills-mcp') {
        router.push('/platform/skills-mcp')
