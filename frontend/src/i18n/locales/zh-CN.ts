export default {
  ...
  tenantMember: {
    ...
    sectionDescriptionEnterprise: '当前空间的成员列表。暂时空间成员都是由系统管理员来统一维护（如添加等）。',
  ...
  skillLibrary: {
    ...
    pushResultTitle: '推送结果',
    pushBlockedCount: '被阻止',
    pushNoteInstall: '以上空间已同步技能定义（物化到各自技能目录）。要让用户环境的智能体可选用，还需系统管理员在该空间把技能「安装」到沙箱镜像 —— 推送不等于安装。',
    pushStatus: {
      updated: '已更新',
      healed: '已重建',
    ...
    editTitle: '编辑平台技能',
    registerDescription: '上传 zip 注册一次，之后可分配给多个空间。',
    editDescription: '重新注册技能包（名称不可改）；编辑后需手动「推送更新」传播。',
    nameSectionTitle: '技能名称',
    nameImmutableHint: '技能名称不可修改；如需改名请注册新技能。',
    // ---- 基本信息（分类 / 作者，000104 / 000105）----
    metaSectionTitle: '基本信息',
    metaSectionHint: '分类需先在「分类管理」中创建，这里只能选择已有分类；作者留空时注册自动读取 SKILL.md。中文名称 / 描述为列表展示用，留空则回落 SKILL.md 内容。',
    categoryLabel: '分类',
    categoryPlaceholder: '选择分类（未分类留空）',
    authorLabel: '作者',
    authorPlaceholder: '留空自动读取 SKILL.md',
    ...
    categoryManagerHint: '分类先创建再使用：输入名称新建，注册或编辑技能时只能从中选择。重命名会移动名下技能；删除会将其下技能置为未分类；两类改动都点亮待推送标记，需推送后进入空间。',
    categoryManagerEmpty: '暂无分类。输入名称创建第一个分类。',
    categoryManagerNewLabel: '新分类名称',
    categoryManagerNewPlaceholder: '输入分类名称',
    categoryManagerCreate: '创建分类',
    categoryManagerCreateFailed: '创建分类失败',
    categoryManagerCount: '{count} 个技能',
    categoryManagerRenameTo: '重命名「{name}」为',
    categoryManagerRenameSave: '保存',
    categoryManagerRenameCancel: '取消',
    categoryManagerRemove: '删除分类',
    categoryManagerRemoveConfirmBody: '确定删除分类「{name}」吗？该分类下的 {count} 个技能将变为未分类。',
    assignmentsSection: '空间分配',
    assignmentsSectionDesc: '分配会在每个空间物化一份独立的技能定义（空间可自行安装到沙箱）；编辑技能后需手动「推送更新」传播。',
    noPlatformTenants: '暂无可分配的空间',
    assignSwitchLabel: '分配到该空间',
    assignmentSkillsHint: '已装 {count} 个沙箱，取消分配前需先在空间内卸载',
    assignmentNotInstalledHint: '仅同步了定义，尚未安装到该空间沙箱',
    assignmentBlockedTitle: '以下空间的操作被拒绝：',
    ...
    toasts: {
      ...
      categoryRenamed: '已重命名 {count} 个技能的分类',
      categoryRemoved: '已将 {count} 个技能移出分类',
      categoryOpFailed: '分类操作失败',
    },
  },
  mcpServiceDialog: {
    addTitle: '添加 MCP 服务',
    ...
    category: '分类',
    categoryPlaceholder: 'Skills/MCP 浏览页的分组，留空为未分类',
    transportType: '传输类型',
    serviceUrl: '服务 URL',
    ...
    globalSettings: {
      ...
      audit: {
        ...
        action: {
          ...
          'system.user_created': '开新账户',
          'system.user_updated': '更新用户资料',
          'system.user_enabled': '启用用户',
          'system.user_disabled': '停用用户',
          'system.user_deleted': '删除用户',
          'system.tenant_updated': '编辑/启停空间',
  ...
  systemConsole: {
    ...
    menuGroups: {
      workspace: '空间与用户',
      // 模型分组：模型管理 + AI用量统计（原「模型与运行时」，Ollama 运行时
      // 已暂时从菜单隐藏）。
      runtime: '模型',
    ...
    models: {
      title: '模型管理',
      description: '平台统一配置对话、嵌入、排序、多模态与语音模型，再分配给各工作空间使用。',
    },
    ollama: {
      title: 'Ollama 运行时',
      description: 'Ollama 本地运行时是全局单例，供本地来源模型使用。',
    },
    assignments: {
      open: '模型分配',
      title: '模型分配 · {name}',
      hint: '勾选该工作空间可用的非内置模型；内置模型对所有空间开放，无需分配。移除仍被引用的模型会被拒绝。',
      selectAll: '全选',
      noModelsOfType: '该类型暂无自定义模型',
      saved: '模型分配已更新',
    },
    users: {
      title: '用户管理',
      description: '管理平台账号的开通、启用状态与工作空间归属。',
      ...
      revoke: '撤销系统管理员',
      deleteUser: '删除用户',
      disableConfirm: '停用后「{name}」将无法登录且现有会话全部失效，确认停用？',
      promoteConfirm: '「{name}」将获得平台级管理权限（管理所有用户与空间），确认提升？',
      revokeConfirm: '撤销后「{name}」不再是系统管理员，确认撤销？',
      deleteUserConfirm: '将硬删除用户「{name}」（工号 {employeeId}）：其所有空间绑定会被移除，且无法恢复。确认删除？',
      colEmployeeId: '工号',
      colUsername: '姓名',
      colState: '状态',
      colBindings: '绑定空间',
    ...
    create: {
      ...
      submit: '创建账户',
      employeeId: '工号',
      employeeIdHelp: '工号是登录标识，创建后不可修改',
      employeeIdPlaceholder: '如 10001',
      username: '姓名',
      usernamePlaceholder: '2-50 个字符',
      password: '初始密码',
      passwordPlaceholder: '默认 abc123',
      defaultPasswordHelp: '默认初始密码为 abc123，可按需修改',
      defaultPasswordNote: '初始密码',
      passwordRequired: '请填写初始密码',
      generate: '生成',
      tenants: '初始绑定空间',
      tenantsPlaceholder: '选填，可稍后在空间绑定中配置',
      missingRequired: '请填写工号和姓名',
      successTitle: '账户已创建',
      bindFailed: '绑定失败',
    },
    edit: {
    ...
    reset: {
      title: '重置密码：{name}',
      hint: '重置后该用户的所有会话将被吊销，下次登录需使用新密码并强制修改。',
      newPassword: '新密码',
      required: '请填写新密码',
      submit: '重置',
      success: '密码已重置',
    ...
    bindings: {
      ...
      empty: '尚未绑定任何空间',
      add: '绑定',
      addPlaceholder: '选择要绑定的空间',
      rolePlaceholder: '绑定角色',
      remove: '解绑',
      removeConfirm: '将「{name}」从空间「{tenant}」移除，确认解绑？',
    },
    tenants: {
      title: '工作空间',
      description: '管理企业内的各工作空间及其成员，并为其分配可用模型。',
      hint: '空间由系统管理员统一创建，成员通过用户管理的空间绑定加入。',
      create: '新建空间',
      edit: '编辑',
      editTitle: '编辑空间：{name}',
      enabled: '已启用',
      ...
      enable: '启用',
      disable: '停用',
      enableConfirm: '「{name}」将重新开放给其成员使用，确认启用？',
      disableConfirm: '停用后「{name}」的成员将无法使用该空间（数据保留，可随时重新启用），确认停用？',
      delete: '删除空间',
      deleteConfirm: '将删除空间「{name}」。仅没有任何成员的空空间可被删除，且操作不可恢复，确认删除？',
      deleteSuccess: '空间已删除',
      name: '空间名称',
      namePlaceholder: '1-100 个字符',
      nameRequired: '请填写空间名称',
      descriptionPlaceholder: '选填',
      quota: '存储配额（GB）',
      quotaPlaceholder: '留空使用系统默认配额',
      colName: '空间名称',
      colStatus: '状态',
      colDescription: '描述',
    ...
    members: {
      title: '成员管理：{name}',
      open: '成员',
      hint: '指定空间管理员；未指定的成员均为普通用户（可使用空间，不可管理配置）。',
      searchPlaceholder: '搜索工号 / 姓名 / 邮箱',
      colMember: '成员',
      colRole: '角色',
      colJoinedAt: '加入时间',
      roleAdmin: '空间管理员',
      ...
      setMember: '设为普通用户',
      remove: '移出空间',
      removeConfirm: '将「{name}」从空间「{tenant}」移除，确认？',
      lastAdminError: '空间至少保留一名空间管理员，请先指定其他管理员。',
      empty: '该空间暂无成员',
    },
  },
  ...
  auth: {
    ...
    // 企业登录标识：登录卡片用工号，注册/资料等场景仍用邮箱 key
    employeeId: '工号',
    ...
    mustChangePassword: {
      title: '首次登录请修改密码',
      description: '当前密码由管理员设置。为保障账号安全，请设置新的登录密码后再继续使用。',
      submit: '修改密码并重新登录',
      success: '密码修改成功，请使用新密码重新登录',
      backToLogin: '返回登录',
      interceptorHint: '请先修改密码，再访问其他功能',
    },
    workspaceOnboarding: {
      enterpriseTitle: '等待分配工作空间',
      enterpriseDescription: '您的账号尚未关联任何工作空间。请联系系统管理员为您分配空间访问权限。',
      enterpriseHelp: '分配空间后，您即可访问空间内的知识库与智能体资源。'
    }
  },
  initialization: {
    skip: '跳过',
  ...
  settings: {
    ...
    skills: {
      ...
      // 000108：空间内显示/隐藏开关
      hiddenInSpace: '本空间内隐藏',
      spaceVisibleAria: '在本空间显示或隐藏此技能',
      showToSpace: '已在本空间显示',
      hideFromSpace: '已在本空间隐藏，将不出现在 Skills/MCP 列表与智能体引用中',
      ...
      // 单个就绪安装：不再写沙箱名，对用户只说「装到了本空间沙箱」
      installedOnSandbox: '已安装到本空间沙箱',
      // ready 但被停用（沙箱侧 disable）——避免被误读成「没装」
      disabledOnSandbox: '已停用',
      ...
      loadFailed: '加载失败',
      // 分类（Skills/MCP 浏览页分组）：随平台分配物化，卡片上系统管理员可改
      category: '分类',
      categoryPlaceholder: 'Skills/MCP 浏览页的分组，留空为未分类',
      editCategory: '修改分类',
      categorySaved: '分类已更新',
    ...
    weknoraCloud: {
      // 000094 模型平台化后仅保留凭证文案：模型侧表单在
      // ModelEditorDialog，解析引擎凭证内联在 ParserEngineSettings。
  ...
  agent: {
    ...
    editor: {
      ...
      skillNotInstalledSelf: '未安装到此沙箱，点击「安装」即可使用',
      skillNotInstalledByAdmin: '未安装到此沙箱，需由本空间管理员或系统管理员安装后可用',
      skillNotReady: '尚未就绪',
  ...
  skillsMcp: {
    title: 'Skills/MCP',
    subtitle: '查看空间可用的Skills（技能）和MCP',
    tabs: {
      skills: 'Skills',
      mcp: 'MCP'
    },
    ...
    category: '分类',
    categoryPlaceholder: '输入分类名称，留空清除',
    creator: '创建者',
    systemCreator: '系统',
    createdAt: '创建时间',
    basicInfo: '基本信息',
    ...
    noReferencingAgents: '暂无智能体引用',
    editCategory: '编辑分类',
    categorySaved: '分类已更新',
    categorySaveFailed: '分类更新失败，请稍后再试',
    loadFailed: '加载失败，请稍后刷新重试',
    empty: {
      skills: '暂无可用技能',
      mcp: '暂无可用 MCP 服务',
      allDesc: '技能目录与 MCP 服务由空间管理员配置',
      favorites: '暂无收藏',
      favoritesDesc: '在卡片右上角点击星标即可收藏',
      recents: '暂无最近访问',
      recentsDesc: '最近打开过的资源会出现在这里',
      search: '未找到匹配项',
      searchDesc: '换个关键词试试'
    }
  },
